const STORAGE_KEY = "tb-losari-jaya-pos-demo-v1";
const DEMO_CREDENTIALS = {
  username: "admin",
  password: "losari123"
};
const POS_BRAND = window.POS_BRAND || {};
const BRAND_LOGO = POS_BRAND.logo || "assets/logolj2.png";
...
function createSeedState() {
  ...
  const salesSpecs = [
    { daysAgo: 0, hour: 8, minute: 30, items: [["SMN-001", 2], ["PKU-050", 1], ["PKP-025", 0.5]], discount: 0, payment: 170000 },
...
function loadState() {
  ...
  try {
    ...
    return {
      ...
      categories: Array.isArray(saved.categories) ? saved.categories : seed.categories,
      units: Array.isArray(saved.units) ? saved.units : seed.units,
      goodsIn: Array.isArray(saved.goodsIn) ? saved.goodsIn : seed.goodsIn,
      sales: Array.isArray(saved.sales) ? saved.sales : seed.sales,
      cart: Array.isArray(saved.cart) ? saved.cart : seed.cart,
      lastReceipt: saved.lastReceipt || seed.lastReceipt,
      auth: {
        ...seed.auth,
        ...(saved.auth || {})
      },
...
function updateInventoryMode() {
  ...
  els.openAddItemBtn.classList.toggle("hidden", mode === "masters");
  els.inventoryModeButtons.forEach((button) => {
...
function populateIncomingItemOptions() {
  ...
  if (currentValue && state.inventory.some((item) => item.id === currentValue)) {
...
function handleCategoryMasterSubmit(event) {
  ...
  state.categories = uniqueSorted([...getMasterNames(state.categories), name]);
  els.categoryMasterForm.reset();
  renderAll();
  showToast("success", "Kategori ditambahkan", `${name} masuk ke master kategori.`);
}

function handleUnitMasterSubmit(event) {
  event.preventDefault();
  ...
  state.units = uniqueSorted([...getMasterNames(state.units), name]);
  els.unitMasterForm.reset();
  renderAll();
  showToast("success", "Satuan ditambahkan", `${name} masuk ke master satuan.`);
}
...
function handleLogin(event) {
  ...
  showToast("danger", "Login gagal", "Gunakan akun demo: admin / losari123.");
}
...
function resetDemo() {
  ...
  renderAll();
  showToast("info", "Data demo direset", "Seluruh stok, transaksi, dan laporan kembali ke data awal.");
}
...
function handleInventoryTableClick(event) {
  ...
  if (action === "delete") {
    if (window.confirm(`Hapus barang ${item.name}?`)) {
      state.inventory = state.inventory.filter((current) => current.id !== itemId);
      state.cart = state.cart.filter((entry) => entry.itemId !== itemId);
      renderAll();
      showToast("info", "Barang dihapus", `${item.name} berhasil dihapus dari data master.`);
    }
  }
}

function handleIncomingSubmit(event) {
  event.preventDefault();
  ...
  item.stock = normalizeQuantity(item.stock + quantity);
  item.supplier = selectedSupplier;

  state.goodsIn.unshift({
    id: `IN-${Date.now()}`,
    date: new Date(`${els.incomingDate.value}T09:00:00`).toISOString(),
    itemId: item.id,
    itemName: item.name,
    quantity,
    cost,
  ...
  els.incomingItem.value = item.id;
  renderAll();
  showToast("success", "Barang masuk tercatat", `Stok ${item.name} bertambah ${formatQuantity(quantity)} ${item.unit}.`);
}
...
function handleCartListClick(event) {
  ...
  const cartItem = state.cart.find((entry) => entry.itemId === itemId);
  const item = getItemById(itemId);
...
function checkoutCart() {
  ...
  const date = new Date();
  const nextIndex = state.sales.length + 1;
  const transaction = {
    id: createTransactionNumberFromDate(date.toISOString(), nextIndex),
  ...
  transaction.items.forEach((entry) => {
    const item = getItemById(entry.itemId);
    item.stock = normalizeQuantity(Math.max(0, item.stock - entry.quantity));
  });

  state.sales.unshift(transaction);
  ...
  renderAll();
  showToast("success", "Transaksi berhasil", `${transaction.id} tersimpan dan stok diperbarui otomatis.`);
}
...
function handleItemSubmit(event) {
  ...
  const duplicate = state.inventory.find((item) => item.sku === payload.sku && item.id !== previousId);
  if (duplicate) {
    showToast("danger", "SKU sudah digunakan", `SKU ${payload.sku} sudah dipakai oleh ${duplicate.name}.`);
    return;
  }

  const existing = state.inventory.find((item) => item.id === previousId);
  if (existing) {
    Object.assign(existing, payload);
    showToast("success", "Barang diperbarui", `${payload.name} berhasil diperbarui.`);
  } else {
    state.inventory.push(payload);
    showToast("success", "Barang ditambahkan", `${payload.name} berhasil masuk ke data master.`);
  }
